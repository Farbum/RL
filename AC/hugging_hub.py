...
def _evaluate_agent(env, n_eval_episodes, model, seed = 1234):
    ...
    """
    venv = VecNormalize(env, norm_obs=False, norm_reward=False, clip_obs=10)
    episode_rewards = []
    for episode in range(n_eval_episodes):
        ...
        while done == False:
            ep_step += 1
            actions_tensor, _, _= model.act(state, 1, mode = "eval")
            action_numpy = actions_tensor.detach().cpu().numpy()
            obs,  reward, done, truncated  = venv.step(action_numpy[0])
            new_state = np.concatenate([obs["observation"], obs["desired_goal"]], axis=-1)
            total_rewards_ep += reward
            state = new_state
        print(f"    -ep ended at step = {ep_step}")
...
def record_video(env, model, out_directory, fps=30, seed = 1234):
    ...
    for num_episode in range(3):
        ...
        while done == False:
            ...
            img = env.render(mode='rgb_array')
            images.append(img)
        print(f"    -Episode finished in {ep_step} steps")
...
def _generate_model_card(model_name, env_id, mean_reward, std_reward, hyperparameters):
    ...
    model_card = f"""
  # Actor-Critic Agent Playing {env_id}

  This is a trained model of an AC agent playing {env_id}.
